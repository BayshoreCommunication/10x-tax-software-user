...
const EmailUpdateForm = ({
  ...
  const [otpVerifyFlag, setOtpVerifyFlag] = useState<boolean>(false);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    // Ensure newPassword and oldPassword are filled
    if (!email) {
    ...
    try {
      ...
      const response = await updateUserEmail({ email } as any);
